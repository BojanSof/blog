...
Creating the `qBitTorrent` LXC is straight-forward, using [helper script](https://community-scripts.github.io/ProxmoxVE/scripts?id=qbittorrent).
After creating the LXC, we need to mount the external drive in the LXC.
The easiest way to do this is to edit the `/etc/pve/lxc/<LXC_ID>.conf` file and add mount point.
For example, if the LXC ID is 106, and we have the path `/mnt/storage/torrents` and we want to mount it in path `/torrents` in the LXC, we can add the following line in the `/etc/pve/lxc/106.conf` file:
```
...
mp0: /mnt/storage/torrents,mp=/torrents
...
```
{: file='/etc/pve/lxc/106.conf}

To host the other apps, I've created VM with [Debian 13](https://www.debian.org/News/2025/20250809), without DE, with 2 vCPUs, 2 GB RAM and 16 GB of storage for the OS.
I've also mounted the shared folders via NFS, from the external drive, by editting `/etc/fstab` of the VM:
...
The easiest way to deploy the applications is using [`docker`](https://www.docker.com/).
We are going to use [Docker Compose](https://docs.docker.com/compose/) for running the multiple containers - one for each app.
I've utilized the following project structure:
...
└── docker-compose.yml
```
{: file="music-project-structure"}

The config folder actually will contain all necessary config files for the containers, so although initially some of them are empty, they will be populated.

The `docker-compose.yml` file can look as follows.
The paths and IDs can be adjusted per user needs.
...
To start the containers, we can run the command `docker compose up -d`, that will start the containers in background.
We will need to configure them before we can use them.
First, let's configure `Lidarr`.
After opening the web interface, we go to `Settings -> Media Management` and create new root folder that will contain our music library.
In my case, that's the `/music` directory, which is actually mapped to `/mnt/storage/media/music` directory on the external drive.
On the same `Media Management` page, we can give naming convention for the tracks, so every track in our music library is saved following the same naming pattern.
On the `Profiles` settings page, we can define `Metadata` profile to use, that will tell `Lidarr` which music types to search for.
Currently I have single profile, called `Standard` that tells `Lidarr` to fetch only Official Studio Albums.
On the `Download Clients` page we can add clients to be used for downloading the music.
I have added the `qBitTorrent` client, by specifying its IP address, port and the credentials for logging to the client.
`qBitTorrent` can fetch music from the torrent indexers provided from `Prowlarr`, after we configure it.
`slskd` is also a client that we will use, but it doesn't have direct integration.
Instead, we will configure the `Soularr` script, which provides the communication between `Lidarr` and `slskd`.
Before carrying on with other configurations, we will need the `Lidarr` API key for later, which can be found under `Settings -> General` page.
...
Creating personal homelab is a fun and educational project.
You can learn a lot of stuff and it doesn't take a lot of money to start one.
Old PC or single board computer and router are all you need to get one started.
Then, over time, the hardware can be upgraded, depending on someone needs.
Beside the educational part, it can also be quite practical - from managing media files, blocking ads and even hosting personal projects and applications.
I've covered some tips and ideas from the homelab setup that I've started which hopefully can inspire others to do the same.